        ...
        this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
            tools: [
                ...
                // Windows-MCP Integration Tools (Windows hosts only)
                {
                    ...
                    description: 'Windows only. One-shot setup of the Windows-MCP server: install mkcert (scoop -> winget -> choco -> openssl fallback), generate locally-trusted TLS certs + an auth key, then launch over secure streamable-http. Runs the PowerShell integration non-interactively.',
                    inputSchema: {
                        type: 'object',
                        properties: {
                ...
                },
                {
                    ...
                    description: 'Windows only. Idempotently launch the Windows-MCP server once (single instance) over streamable-http with TLS. No-ops if already running.',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            transport: { type: 'string', enum: ['streamable-http', 'sse'], description: 'Transport (default: streamable-http)' },
