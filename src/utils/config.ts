...
import { WindowsMCPConfig, WindowsMCPTransport } from '../types/zellij.js';
...
const WINDOWS_MCP_DEFAULTS: WindowsMCPConfig = {
...
function readFileConfig(): Partial<ZellijMCPConfig> {
  const configPath = getConfigPath();
  if (!existsSync(configPath)) {
    return {};
  }
  try {
    return JSON.parse(readFileSync(configPath, 'utf8')) as Partial<ZellijMCPConfig>;
  } catch (error: any) {
...
function normaliseTransport(value: unknown): WindowsMCPTransport | undefined {
...
export function loadConfig(): ZellijMCPConfig {
  const cached = cache.get<ZellijMCPConfig>(CONFIG_CACHE_KEY);
  if (cached) {
    return cached;
  }

  const fileConfig = readFileConfig();
  const merged: ZellijMCPConfig = {
...
export function resolveWindowsMCPConfig(overrides?: Partial<WindowsMCPConfig>): WindowsMCPConfig {
